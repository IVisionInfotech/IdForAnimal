...
public class AnimalListAdapter extends RecyclerView.Adapter<AnimalListAdapter.MyViewHolder> {
    ...
    public AnimalListAdapter(Context context, ArrayList<Animal> list, ClickListener listener) {
    ...
    public AnimalListAdapter(Context context, String id, ArrayList<Animal> list, ClickListener listener) {
        this.context = context;
        this.session = new Session(context);
        this.id = id;
        this.list = list;
    ...
    public AnimalListAdapter(Context context, boolean isShow, ArrayList<Animal> list, ClickListener listener) {
        this.context = context;
        this.session = new Session(context);
        this.isShow = isShow;
        this.list = list;
        ...
        if (holder.getItemViewType() == VIEW_TYPE_ITEM) {
            if (holder instanceof MyViewHolder) {
                ...
                if (!isShow) {
                    ...
                    mainHolder.binding.ivCall.setVisibility(View.VISIBLE);
                ...
                mainHolder.binding.ivEdit.setOnClickListener(v -> {
                    if (listener3 != null) listener3.onItemSelected(position);
                });
                mainHolder.binding.ivDelete.setOnClickListener(v -> {
                    if (listener4 != null) listener4.onItemSelected(position);
                });
                mainHolder.binding.ivMore.setOnClickListener(v -> {
                    if (listener != null)
                        Common.openPopupMenu(context, mainHolder.binding.ivMore, R.menu.animal_menu, new ClickListener() {
                            @Override
                            public void onItemSelected(int itemId) {
                                listener.onItemSelected(position, itemId);
                            }
                        });
                });
